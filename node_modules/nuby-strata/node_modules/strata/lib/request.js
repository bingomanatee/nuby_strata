...
Request.prototype.body = function body(callback) {
    if (this.env.strataBody) {
        console.log('body callback: %s', callback.toString);
        callback(null, this.env.strataBody);
        return;
    }
    console.log('body is happening');
    ...
    if (this.mediaType === "application/json") {
       // console.log('jsonning');
        ...
        input.on("end", function () {
            try {
                //  console.log('Request.prototype.body::parsing body: %s', body);
                var obj = JSON.parse(body);
            } catch (e) {
                console.log('Request.prototype.body::parsing body: %s FAIL %s', body, e.message);
       ...
       // console.log('body parsable');
        ...
        if (match) {
            ...
            try {
                ...
                callback(new strata.Error(message, e), params);
                return;
            }
       ...
       // console.log('no parsing');
        ...
        //console.log('input reading: %s', util.inspect(input));
        input.on("data", function (buffer) {
            console.log('data');
            body += buffer.toString("utf8");
        });
...
Request.prototype.params = function params(callback) {
    if (this.env.strataParams) {
        console.log('returning strataParams');
    ...
    this.query(function (err, query) {
        console.log('query : %s, callback: %s, err: %s', util.inspect(query), callback.toString(), err);
        if (err) {
            callback(err, params);
        } else {
            ...
            self.body(function (err, body) {
                ...
                if (err) {
                    console.log('body err: %s', err.toString());
                    callback(err, params);
                } else {
                    ...
                    console.log('calling back %s', util.inspect(params));
                    callback(null, params);
                }
            });
        }
