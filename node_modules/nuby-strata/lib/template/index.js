...
Template.prototype = {
    ...
    render:function (env, body, callback, action) {
        ...
        function _render(){
            template.context.layout(template.template_file_path(), env, body, callback, action);
        }

        /**
         * on_render is a function that modifies _render_params property of env;
         * when complete, _render action is called.
         */

        if (this.hasOwnProperty('on_render') && _.isFunction(this.on_render)){
            template.on_render(env, _render);
        } else {
            _render();
        }
    }
