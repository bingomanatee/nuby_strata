...
Action.prototype = {
    ...
    _on_data:function (err, data, env, app_callback) {
        ...
        if (action.context && action.context.hasOwnProperty('tracer')) {
        ...
        this.render(env, function (err, content) {
            action._on_render(err, content, env, app_callback);
        })
    },

    /**
     * this is the final reinsertion of the result of the action into the app callback.
     *
     * @param err
     * @param content
     * @param env
    ...
    _on_render:function (err, content, env, app_callback) {
        if (err) {
            return this.error(env, {err:err, type:'data_error'}, app_callback);
        } else {
            app_callback(200, this.header(env), content);
        }
    },
    ...
    render:function (env, on_render) {
        ...
        /**
         * note - formatted rendering sidesteps
         * the do_render method which
         * adds the layout around the content.
         */
