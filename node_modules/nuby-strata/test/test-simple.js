var mock = require('../node_modules/strata/lib/mock.js');
var util = require('util');

module.exports = {
    setUp:function (setup_callback) {
        var app_maker = require('./../test_resources/simple_server/app.js');
        app_maker(function (err, context) {
            module.exports.app = context.app;
            //  console.log('setup done; app = %s', util.inspect(app));
            setup_callback();
        });
    },

    test_simple:function (test) {
        mock.request('/foo_path', module.exports.app, function (err, status, headers, body) {
            //   _echo(err, status, headers, body);

            var content_type = 'text/plain';
            var content = 'simple world';

            test.equal(body, content, 'body should equal ' + content);
            test.equal(headers['Content-Type'], content_type,
                'content-type should equal ' + content_type);
            test.done();
        });
    },

    test_simple_json:function (test) {
        mock.request('/foo_path.json', module.exports.app, function (err, status, headers, body) {
            var content_type = 'application/json';
            var content = '{"foo":1}';

            test.equal(body, content, 'body should equal ' + content);
            test.equal(headers['Content-Type'], content_type,
                'content-type should equal ' + content_type);
            test.done();
        });
    },

    test_action:function (test) {
        mock.request('/bar_path', module.exports.app, function (err, status, headers, body) {
            //   _echo(err, status, headers, body);

            var content_type = 'text/plain';
            var content = 'barville';

            test.equals(body, content, 'body should equal ' + content);
            test.equals(headers['Content-Type'], content_type,
                'content-type should equal ' + content_type);
            test.done();
        });
    },

    test_action_json:function (test) {
        mock.request('/bar_path.json', module.exports.app, function (err, status, headers, body) {
            var content_type = 'application/json';
            var content = '{"bar":10}';

            test.equals(body, content, 'body should equal ' + content);
            test.equals(headers['Content-Type'], content_type,
                'content-type should equal ' + content_type);
            test.done();
        });
    }

}

function _echo(err, status, headers, body) {
    console.log('err: ', util.inspect(err));
    console.log('status', util.inspect(status));
    console.log('headers', util.inspect(headers));
    console.log('body', util.inspect(body));
}